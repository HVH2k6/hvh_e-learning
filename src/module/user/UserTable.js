...
const UserTable = () => {
  ...
  const renderLabelStatus = (status) => {
    switch (status) {
      case userStatus.ACTIVE:
        return <LabelStatus type="success">Duyệt </LabelStatus>;
      case userStatus.PENDING:
        return <LabelStatus type="warning">Chờ duyệt</LabelStatus>;
      case userStatus.BAN:
        return <LabelStatus type="danger">Từ chối</LabelStatus>;
  ...
  const renderUserItem = (user) => {
    console.log("file: UserTable.js:79 ~ renderUserItem ~ user:", user)
    return (
      <tr key={user.id}>
